...
void vkey_map_init(void) {
    ...
    vkey_mem_root = memalign(getpagesize() * 512, VKEY_PAGE_NUM * getpagesize() * 512); // alignment, size
    memset(vkey_mem_root, 0, VKEY_PAGE_NUM * getpagesize() * 512);
    ...
    for(i = 0; i < VKEY_PAGE_NUM; i++) {
        ...
        void *addr = (void *)((unsigned long int)vkey_mem_root + (unsigned long int)getpagesize() * 512 * i);
        // printf("i: %d, vkey: %d, addr: %lx\n", i, vkey, (unsigned long int)addr);
        vkey_mprotect(addr, getpagesize() * 512, vkey);
    }
...
void * vkey_to_addr(int vkey) {
    int index = vkey - 1;
    return (void *)((unsigned long int)vkey_mem_root + index * getpagesize() * 512);
}

int addr_to_vkey(void *addr) {
    int index = (int)(((unsigned long int)addr - (unsigned long int)vkey_mem_root) / (getpagesize() * 512));
    return index + 1;
}
...
EVP_PKEY *EVP_PKEY_new(void)
{
    ...
    // printf("[openssl] new vkey, addr: %lx, vkey: %d, thread[%d]\n", (long unsigned int)ret, vkey, (int)syscall(SYS_gettid));
    // printf("[openssl] EVP_PKEY_new, before memset\n");
    memset((void *)ret, 0, sizeof(*ret));
    // printf("[openssl] EVP_PKEY_new, after memset\n");
 ...
 err:
    ...
    // printf("[openssl] free vkey, addr: %lx, vkey: %d, thread[%d]\n", (long unsigned int)ret, vkey, (int)syscall(SYS_gettid));
...
void EVP_PKEY_free(EVP_PKEY *x)
{
    ...
    wrvkru(vkey, VKEY_AD);
    vkey_release_vkey(vkey);
    // printf("[openssl] free vkey, addr: %lx, vkey: %d, thread[%d]\n", (long unsigned int)x, vkey, (int)syscall(SYS_gettid));
}
